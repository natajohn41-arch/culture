      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <dl class="row">
            ...
            <dd class="col-sm-8">{{ $region->localisation ?? '-' }}</dd>

            <dt class="col-sm-4">Superficie</dt>
            <dd class="col-sm-8">{{ $region->superficie ?? '-' }}</dd>

            <dt class="col-sm-4">Population</dt>
            <dd class="col-sm-8">{{ $region->population ?? '-' }}</dd>

            <dt class="col-sm-4">Description</dt>
            <dd class="col-sm-8">{{ $region->description ?? '-' }}</dd>
          </dl>

          <div class="d-flex justify-content-between">
            <a href="{{ route('region.index') }}" class="btn btn-secondary" title="Retour" aria-label="Retour"><i class="bi bi-arrow-left"></i></a>

            <div>
              <a href="{{ route('region.edit', $region->id_region) }}" class="btn btn-primary me-2" title="Éditer" aria-label="Éditer"><i class="bi bi-pencil"></i></a>

              <form action="{{ route('region.destroy', $region->id_region) }}" method="POST" class="d-inline" onsubmit="return confirm('Supprimer cette région ?');">
                @csrf
                @method('DELETE')
