        ...
        <div class="card-body">
          ...
          <div class="mb-3 text-center">
            @if(!empty($utilisateur->photo))
              <img src="{{ asset('storage/' . $utilisateur->photo) }}" alt="Photo" class="img-thumbnail" style="max-width:180px; height:auto;">
            @else
              <div class="border rounded-circle d-inline-flex align-items-center justify-content-center" style="width:120px; height:120px;">
                <strong>{{ strtoupper(substr($utilisateur->nom ?? '',0,1) . substr($utilisateur->prenom ?? '',0,1)) }}</strong>
              </div>
            @endif
          </div>
          ...
          <dl class="row">
            ...
            <dd class="col-sm-8">{{ $utilisateur->nom ?? '-' }}</dd>

            <dt class="col-sm-4">Prénom</dt>
            <dd class="col-sm-8">{{ $utilisateur->prenom ?? '-' }}</dd>

            <dt class="col-sm-4">Email</dt>
            <dd class="col-sm-8">{{ $utilisateur->email ?? '-' }}</dd>

            <dt class="col-sm-4">Rôle (ID)</dt>
            <dd class="col-sm-8">{{ $utilisateur->id_role ?? '-' }}</dd>

            <dt class="col-sm-4">Langue (ID)</dt>
            <dd class="col-sm-8">{{ $utilisateur->id_langue ?? '-' }}</dd>

            <dt class="col-sm-4">Sexe</dt>
            <dd class="col-sm-8">{{ $utilisateur->sexe ?? '-' }}</dd>

            <dt class="col-sm-4">Statut</dt>
            <dd class="col-sm-8">{{ $utilisateur->statut ?? '-' }}</dd>

            <dt class="col-sm-4">Date de naissance</dt>
            <dd class="col-sm-8">{{ optional($utilisateur->date_naissance)->format('Y-m-d') ?? $utilisateur->date_naissance ?? '-' }}</dd>
          </dl>

          <div class="d-flex justify-content-between">
            <a href="{{ route('utilisateurs.index') }}" class="btn btn-secondary" title="Retour" aria-label="Retour"><i class="bi bi-arrow-left"></i></a>

            <div>
              <a href="{{ route('utilisateurs.edit', $utilisateur->id_utilisateur) }}" class="btn btn-primary me-2" title="Éditer" aria-label="Éditer"><i class="bi bi-pencil"></i></a>

              <form action="{{ route('utilisateurs.destroy', $utilisateur->id_utilisateur) }}" method="POST" class="d-inline" onsubmit="return confirm('Supprimer cet utilisateur ?');">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger" title="Supprimer" aria-label="Supprimer"><i class="bi bi-trash"></i></button>
              </form>
            </div>
          </div>
