      ...
      <main class="app-main">
          <div class="card mb-4">
                  <div class="card-header d-flex justify-content-between align-items-center">
                    <h3 class="card-title mb-0"><strong>Listes des Utilisateurs</strong></h3>
                    <a href="{{ route('utilisateurs.create') }}" class="btn btn-primary" title="Ajouter un utilisateur" aria-label="Ajouter un utilisateur"><i class="bi bi-plus-lg"></i></a>
                  </div>
                  <!-- /.card-header -->
                      ...
                      <tbody>
                        ...
                        <tr class="align-middle">
                          ...
                          <td>{{ $utilisateur->email }}</td>
                            ...
                            <td>{{ $utilisateur->id_langue }}</td>
                            <td>{{ $utilisateur->id_role }}</td>
                            <td>{{ $utilisateur->sexe }}</td>
                            <td>
                              @if($utilisateur->photo)
                              <img src="{{ asset('storage/photos/' . $utilisateur->photo) }}" alt="Photo de {{ $utilisateur->nom }}" width="50" height="50">
                              @else
                              N/A
                              @endif
                             </td>
                            ...
                            <td>
                            <a href="{{ route('utilisateurs.show', $utilisateur->id_utilisateur) }}" class="btn btn-sm btn-info" title="Afficher" aria-label="Afficher"><i class="bi bi-eye"></i></a>
                            <a href="{{ route('utilisateurs.edit', $utilisateur->id_utilisateur) }}" class="btn btn-sm btn-secondary" title="Modifier" aria-label="Modifier"><i class="bi bi-pencil"></i></a>
                            <form action="{{ route('utilisateurs.destroy', $utilisateur->id_utilisateur) }}" method="POST" style="display:inline-block" onsubmit="return confirm('Supprimer cette langue ?');">
                              @csrf
                              @method('DELETE')
                              <button type="submit" class="btn btn-sm btn-danger" title="Supprimer" aria-label="Supprimer"><i class="bi bi-trash"></i></button>
                            </form>
                          </td>
                        </tr>
                        @empty
                        <tr>
                          <td colspan="4">Aucun Utilisateur trouvée.</td>
                        </tr>
                        @endforelse
                      </tbody>
                    </table>
