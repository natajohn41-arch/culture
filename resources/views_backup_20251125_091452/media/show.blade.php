      ...
      <main class="app-main">
        <div class="container d-flex align-items-center" style="min-height:60vh;">
          <div class="row justify-content-center w-100">
            <div class="col-md-8">
              <div class="card">
                <div class="card-header">Détails du média</div>
                <div class="card-body text-center">
                  @if(!empty($media->chemin))
                    <div class="mb-3">
                      <img src="{{ asset($media->chemin) }}" alt="media" class="img-fluid" style="max-height:320px;">
                    </div>
                  @endif

                  <dl class="row text-start">
                    <dt class="col-sm-4">Chemin</dt>
                    <dd class="col-sm-8">{{ $media->chemin ?? '-' }}</dd>

                    <dt class="col-sm-4">Type (ID)</dt>
                    <dd class="col-sm-8">{{ $media->id_type_media ?? '-' }}</dd>

                    <dt class="col-sm-4">Contenu (ID)</dt>
                    <dd class="col-sm-8">{{ $media->id_contenu ?? '-' }}</dd>

                    <dt class="col-sm-4">Description</dt>
                    <dd class="col-sm-8">{{ $media->description ?? '-' }}</dd>
                  </dl>

                  <div class="d-flex justify-content-between">
                    <a href="{{ route('media.index') }}" class="btn btn-secondary">Retour</a>
                    <div>
                      <a href="{{ route('media.edit', $media->id_media) }}" class="btn btn-primary me-2">Éditer</a>
                      <form action="{{ route('media.destroy', $media->id_media) }}" method="POST" class="d-inline" onsubmit="return confirm('Supprimer ce média ?');">
                        @csrf
                        @method('DELETE')
